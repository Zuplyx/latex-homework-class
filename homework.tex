...
\newpage
\section*{Using \texttt{tikz} to draw a node graph}
	This is a sample node graph.
	\begin{figure}[H]
			\floatbox[{\capbeside\thisfloatsetup{capbesideposition={right,top},capbesidewidth=4cm}}]{figure}[\FBwidth]
			{\caption{A graph with a description on the side}\label{fig:G}}
	{\begin{tikzpicture} 
	% a is the name of the node and A is the text inside the node/vertex  
	\node[state] (a) at (0,0) {$A$}; % here, state signifies that the shape of the node will be the shape declared in the state/.style command in the cls file.  
	...
	\path[->] 	(a) edge (b) % define the paths of the edges from one node to another  
			 	(a) edge node[el,right, rotate=90]  {$-1$} (c) % this edge is labeled
				(a) edge (d)
				(e) edge (f)
	 			(d) edge (f)
 				(b) edge (e)
